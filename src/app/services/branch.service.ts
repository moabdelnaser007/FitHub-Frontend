...
export class BranchService {
  private apiUrl = 'http://localhost:5024/api/owner/Branch';
  ...
  getAllBranches(): Observable<BranchData[]> {
    return this.http.get<BranchResponse>(`${this.apiUrl}/GetAllBranches`, {
  ...
  getBranchById(id: number): Observable<BranchData> {
    return this.http.get<SingleBranchResponse>(`${this.apiUrl}/GetBranchById/${id}`, {
      headers: this.getHeaders()
    }).pipe(
  ...
  updateBranch(id: number, branchData: UpdateBranchRequest): Observable<BranchData> {
    return this.http.put<UpdateBranchResponse>(
  ...
  deleteBranch(id: number): Observable<boolean> {
    return this.http.delete<DeleteBranchResponse>(
      `${this.apiUrl}/DeleteGymBranch?id=${id}`,
      { headers: this.getHeaders() }
    ).pipe(
